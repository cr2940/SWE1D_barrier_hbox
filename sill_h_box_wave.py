...
import shallow_1D_redistribute_wave#_MB for double hbox method
...
def setup(kernel_language='Python',use_petsc=False, outdir='./_output', plotdir='./_plots',solver_type='classic'):
    ...
    state.problem_data['method'] = 'h_box_wave' #h_box for double hbox method; h_box_wave for LTS
    state.problem_data['zero_width'] = True
    state.problem_data['arrival_state'] = False


    solver = pyclaw.ClawSolver1D(shallow_1D_redistribute_wave.shallow_fwave_hbox_dry_1d)  #shallow_1D_redistribute_wave_MB for dbl hbox
    ...
    state.aux[0,:] = numpy.linspace(-1.2, -0.8, xc.shape[0], endpoint=True)

    state.aux[1,:] = xpxc
    state.aux[1,nw-1] = alpha * xpxc
    state.aux[1,nw] = (1 - alpha) * xpxc
    state.q[0,:] = -0.7- state.aux[0,:]
    state.q[0,:80] += 0.15
    ...
    plot(setplot="./setplot_h_box_wave_2.py",plotdir=plotdir,iplot=False, htmlplot=True, **plot_kargs)
