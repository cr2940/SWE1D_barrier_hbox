...
###########################################
########### ghost state setup #############
    if (L2R==True or R2L==True):
        if q_wall[0,0] <drytol or q_wall[0,2] < drytol:
            q_wall[0,1] = max(hstarL,hstarR) - wall_height #q_wall[0,0],q_wall[0,2]) - wall_height # (other possible ghost states that seem to work similarly)
            q_wall[1,1] = q_wall[0,1]  * max(ustarL,ustarR,key=abs)
        else:
            q_wall[0,1] = 0.5*(hstarL+hstarR) - wall_height #q_wall[0,0]+q_wall[0,2]) - wall_height #
            q_wall[1,1] =q_wall[0,1]  * 0.5*(ustarL+ustarR) #(q_wall[1,0] + q_wall[1,2])/(q_wall[0,0] + q_wall[0,2]) #0.5*(ustarL+ustarR) #q_wall[0,1]  * (ustarL*np.sqrt(hstarL) + ustarR*np.sqrt(hstarR))/(np.sqrt(hstarL)+np.sqrt(hstarR)) #q_wall[0,1]  * 0.5*(ustarL+ustarR)#(ustarL*np.sqrt(hstarL) + ustarR*np.sqrt(hstarR))/(np.sqrt(hstarL)+np.sqrt(hstarR)) #q_wall[0,1]  * (q_wall[1,0] + q_wall[1,2])/(q_wall[0,0] + q_wall[0,2]) # q_wall[0,1]  * 0.5*(ustarL+ustarR)#### # #### # h*_avg * (huL+huR)/(hL+hR)

    # to coincide wall_height = 0 case with no barrier case
    if wall_height == 0.0:
...
############################################
    ...
    ################################################################
    lambs[0] = np.min(s) #s[0,0] #0.5*(s[0,0]+s[0,1]) #0.5*(s[0,0] + s[0,1])
    lambs[1] = 0.5*(np.min(s)+np.max(s))# 0.5*(s[1,0]+s[1,1])#0.5*(s[1,1]+s[1,0])
    lambs[2] =np.max(s) #s[2,1] # 0.5*(s[2,1]+s[2,0]) # 0.5*(s[2,0] + s[2,1])
    ##################################################################
...
##################
    fwave_r, s_r, amdq[:,iw], apdq[:,iw] = redistribute_fwavef(q_hbox[:,1].copy(),q_hbox[:,2].copy(),aux_hbox[0,1].copy(), aux_hbox[0,2].copy(),wall_height,drytol,g,maxiter)
